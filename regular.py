...
def prediction(params, x):
    pred = 0
    for i in range(len(params)):pred += params[i] * np.math.pow(x, i)
    return pred
...
def regularization_adjust(args1,args2,times):
    k=.5
    for x in range(1, times):
        result = (args1 + args2) * k
        args1=result
    return args1

#when  e = 3
e=3
reg_adjusted_1 =[regularization_adjust(prediction(w2,x),prediction(w1,x),e) for x in domain]
 # when e = 2
e=2
reg_adjusted_2 =[regularization_adjust(prediction(w2,x),prediction(w1,x),e) for x in domain]
...
plt.plot(domain,reg_adjusted_1)
plt.plot(domain,reg_adjusted_2)
plt.legend(("training","un-regularized","regularized","reg_adjusted_@e=3","reg_adjusted_@e=2"), loc='lower right')
plt.show()